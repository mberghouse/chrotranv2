...
! structured grid faces
  ...
  PetscInt, parameter, public :: ANY_FACE = 7
...
#include "petsc/finclude/petscdm.h"
  ...
  structured_grid%nlmax = structured_grid%nlx * structured_grid%nly * &
                          structured_grid%nlz
  ...
  structured_grid%ngmax = structured_grid%ngx * structured_grid%ngy * &
                         structured_grid%ngz
...
subroutine StructGridGetBoundaryFaces(structured_grid,imat,nL2G, &
                                      cell_ids,faces,iface,option)
  !
  ...
  type(grid_structured_type), pointer :: structured_grid
  PetscInt :: imat(:) ! ghosted
  PetscInt :: nL2G(:) ! non-ghosted
  PetscInt, pointer :: cell_ids(:)
  PetscInt, pointer :: faces(:)
  PetscInt :: iface
  type(option_type) :: option

  PetscInt, allocatable :: boundary_cells_and_faces(:,:,:,:)
  PetscInt, allocatable :: temp_cell_ids(:)
  PetscInt, allocatable :: temp_faces(:)
  type(grid_structured_type), pointer :: sgrid
  PetscInt :: i, j, k
  PetscInt :: local_id, ghosted_id
  PetscInt :: icell, icount

  sgrid => structured_grid ! shorter name

  if (associated(faces)) then
    option%io_buffer = 'faces array should be null coming into &
                         &StructGridGetBoundaryFaces.'
    call PrintErrMsg(option)
  endif

  allocate(temp_cell_ids(6*sgrid%nlmax))
  temp_cell_ids = 0
  allocate(temp_faces(6*sgrid%nlmax))
  temp_faces = 0

  icount = 0
  do icell = 1, size(cell_ids)
    local_id = cell_ids(icell)
    call StructGridGetIJKFromLocalID(sgrid,local_id,i,j,k)
    ghosted_id = nL2G(local_id)
    if (imat(ghosted_id) > 0) then
      if (iface == WEST_FACE .or. iface == ANY_FACE) then
        ! -x direction
        if (i == 1 .and. sgrid%lxs-sgrid%gxs == 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = WEST_FACE
        elseif (i > 1 .and. imat(ghosted_id-1) <= 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = WEST_FACE
        endif
      endif
      if (iface == EAST_FACE .or. iface == ANY_FACE) then
        ! +x direction
        if (i == sgrid%nlx .and. sgrid%gxe-sgrid%lxe == 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = EAST_FACE
        elseif (i < sgrid%nlx .and. imat(ghosted_id+1) <= 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = EAST_FACE
        endif
      endif
      if (iface == SOUTH_FACE .or. iface == ANY_FACE) then
        ! -y direction
        if (j == 1 .and. sgrid%lys-sgrid%gys == 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = SOUTH_FACE
        elseif (j > 1 .and. imat(ghosted_id-sgrid%nlx) <= 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = SOUTH_FACE
        endif
      endif
      if (iface == NORTH_FACE .or. iface == ANY_FACE) then
        ! +y direction
        if (j == sgrid%nly .and. sgrid%gye-sgrid%lye == 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = NORTH_FACE
        elseif (j < sgrid%nly .and. imat(ghosted_id+sgrid%nlx) <= 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = NORTH_FACE
        endif
      endif
      if (iface == BOTTOM_FACE .or. iface == ANY_FACE) then
        ! -z direction
        if (k == 0 .and. sgrid%lzs-sgrid%gzs == 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = BOTTOM_FACE
        elseif (k > 1 .and. imat(ghosted_id-sgrid%nlxy) <= 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = BOTTOM_FACE
        endif
      endif
      if (iface == TOP_FACE .or. iface == ANY_FACE) then
        ! +z direction
        if (k == sgrid%nlz .and. sgrid%gze-sgrid%lze == 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = TOP_FACE
        elseif (k < sgrid%nlz .and. imat(ghosted_id+sgrid%nlxy) <= 0) then
          icount = icount + 1
          temp_cell_ids(icount) = local_id
          temp_faces(icount) = TOP_FACE
        endif
  ...
  deallocate(cell_ids)
  if (associated(faces)) deallocate(faces)
  if (icount > 0) then
    allocate(cell_ids(icount))
    allocate(faces(icount))
    cell_ids = temp_cell_ids(1:icount)
    faces = temp_faces(1:icount)
  else
    nullify(cell_ids)
    nullify(faces)
  endif
...
end subroutine StructGridGetBoundaryFaces
