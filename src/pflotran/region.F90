...
#include "petsc/finclude/petscsys.h"
  ...
  type, public :: region_type
    PetscInt :: id
    PetscInt :: def_type
    PetscBool :: remove_inactive_cells
...
function RegionCreateWithNothing()
  ...
  region%id = 0
  region%def_type = 0
  region%remove_inactive_cells = PETSC_FALSE
  region%name = ""
  region%filename = ""
  region%i1 = 0
  region%i2 = 0
...
function RegionCreateWithRegion(region)
  ...
  new_region%remove_inactive_cells = region%remove_inactive_cells
...
subroutine RegionRead(region,input,option)
      ...
      case('FACE')
        ...
        select case(word)
          ...
          case('TOP')
            region%iface = TOP_FACE
          case('ALL_BOUNDARY')
            region%iface = ANY_FACE
          case default
            option%io_buffer = 'FACE "' // trim(word) // &
              '" not recognized.'
            call PrintErrMsg(option)
        end select
      case('REMOVE_INACTIVE_GRID_CELLS')
        region%remove_inactive_cells = PETSC_TRUE
      case default
  ...
  if (region%iface == ANY_FACE .and. &
      region%def_type /= DEFINED_BY_POLY_BOUNDARY_FACE) then
    option%io_buffer = 'ANY_FACE is only supported for POLYGONal REGIONs &
      of TYPE BOUNDARY_FACES_IN_VOLUME.'
    call PrintErrMsg(option)
  endif
